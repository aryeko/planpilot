...
from planpilot_v2.cli import _format_summary, _run_sync, build_parser, main
...
def test_build_parser_version_prints_and_exits(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "planpilot_v2" in captured.out
...
async def test_run_sync_delegates_to_sdk(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("planpilot_v2.cli.load_config", lambda _: config)
    monkeypatch.setattr("planpilot_v2.cli.PlanPilot.from_config", _fake_from_config)
...
def test_main_returns_zero_on_success(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("planpilot_v2.cli.asyncio.run", lambda _: None)
...
def test_main_enables_verbose_logging(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("planpilot_v2.cli.logging.basicConfig", _fake_basic_config)
...
def test_main_maps_errors_to_exit_codes(
    ...
    monkeypatch.setattr("planpilot_v2.cli.asyncio.run", _raise)
