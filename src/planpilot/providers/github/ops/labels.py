...
from planpilot.contracts.exceptions import ProviderCapabilityError, ProviderError
...
async def ensure_issue_type(
    provider: GitHubProvider, issue_id: str, item_type: PlanItemType
...
async def ensure_discovery_labels(
    provider: GitHubProvider, issue_id: str, labels: list[str]
) -> None:  # pragma: no cover
...
async def resolve_label_ids(provider: GitHubProvider, label_names: list[str]) -> list[str]:  # pragma: no cover
    ids: list[str] = []
...
async def find_or_create_label(provider: GitHubProvider, name: str) -> str:  # pragma: no cover
    client = provider._require_client()
    ...
    if data.repository and data.repository.labels and data.repository.labels.nodes:
        for node in data.repository.labels.nodes:
            if node and node.name == name:
                return node.id
    return await create_label(provider, provider.context.repo_id, name=name)


async def create_label(provider: GitHubProvider, repo_id: str, *, name: str | None = None) -> str:  # pragma: no cover
    client = provider._require_client()
