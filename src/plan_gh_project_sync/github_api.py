...
def create_issue(repo_id: str, input_value: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    if not repo_id:
        return None
    query = """
...
def add_project_item(project_id: str, content_id: str) -> Optional[str]:
    query = """
    ...
    """
    data = gh_json(['api', 'graphql', '-f', f'query={query}', '-f', f'projectId={project_id}', '-f', f'contentId={content_id}'])
