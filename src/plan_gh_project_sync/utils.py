...
def get_project_item_id(content_id: str, cache: Dict[str, str]) -> Optional[str]:
...
def build_parent_map(nodes: list) -> Dict[str, Optional[str]]:
    mapping = {}
    for node in nodes:
...
def resolve_option_id(options: list, name: str) -> Optional[str]:
    if not name:
        return None
    for opt in options:
