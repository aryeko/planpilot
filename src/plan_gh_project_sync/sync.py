...
def run_sync(config: SyncConfig) -> None:
    ...
    for story in stories:
        ...
        if story.get('task_ids') is None and story.get('story_ids') is None and story_tasks.get(story.get('id'), []) == []:
    ...
    try:
        ...
        if project:
            project_fields = resolve_project_fields(project, config.status, config.priority, config.iteration, config.size_field)
        ...
        if config.label not in names:
            if _dry_run:
                ...
                gh_run([
                    ...
                    '-R', config.repo,
                    '--description', f'{config.label} tracking issue',
                    '--color', '0E8A16',
                ])
        # try again
        if repo_id:
    ...
    def set_project_fields(item_id: str, size_option_id: Optional[str] = None) -> None:
        if not project_id or not project_fields:
            return
        if project_fields.status_field_id and project_fields.status_option_id:
            update_project_field(project_id, item_id, project_fields.status_field_id, {
        ...
        if project_fields.priority_field_id and project_fields.priority_option_id:
            update_project_field(project_id, item_id, project_fields.priority_field_id, {
        ...
        if project_fields.iteration_field_id and project_fields.iteration_option_id:
            update_project_field(project_id, item_id, project_fields.iteration_field_id, {
                'iterationId': project_fields.iteration_option_id,
            })
        if project_fields.size_field_id and size_option_id:
            update_project_field(project_id, item_id, project_fields.size_field_id, {
                'singleSelectOptionId': size_option_id,
            })

    def add_to_project(issue_id: str) -> Optional[str]:
        if not project_id:
            return None
    ...
    for epic in epics:
        ...
        if issue_id and issue_type_ids.get('Epic') and not _dry_run:
            ...
            """
            gh_json(['api', 'graphql', '-f', f'query={query}', '-f', f'id={issue_id}', '-f', f'issueTypeId={issue_type_ids["Epic"]}'])
    ...
    for story in stories:
        ...
        if issue_id and issue_type_ids.get('Story') and not _dry_run:
            ...
            """
            gh_json(['api', 'graphql', '-f', f'query={query}', '-f', f'id={issue_id}', '-f', f'issueTypeId={issue_type_ids["Story"]}'])
        item_id = None if _dry_run else add_to_project(issue_id)
        if item_id:
            set_project_fields(item_id)
    ...
    for task in tasks:
        ...
        if issue_id and issue_type_ids.get('Task') and not _dry_run:
            ...
            """
            gh_json(['api', 'graphql', '-f', f'query={query}', '-f', f'id={issue_id}', '-f', f'issueTypeId={issue_type_ids["Task"]}'])
        item_id = None if _dry_run else add_to_project(issue_id)
        if item_id:
            size_option_id = None
    ...
    if _dry_run:
        ...
        _log_dry("link sub-issues", f"stories under epic, tasks under stories")
        _log_dry("add blocked-by links", f"task dependencies")
        parent_map = {}
        blocked_by_map = {}
    else:
        relation_ids = [data['node_id'] for data in sync_map['tasks'].values()] + [
    ...
    if not _dry_run:
        ...
        for story in stories:
            ...
            """
            gh_json(['api', 'graphql', '-f', f'query={query}', '-f', f'issueId={epic_node_id}', '-f', f'subIssueId={story_node_id}'])
        ...
        for task in tasks:
            ...
            """
            gh_json(['api', 'graphql', '-f', f'query={query}', '-f', f'issueId={story_node_id}', '-f', f'subIssueId={task_node_id}'])
